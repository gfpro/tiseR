...
- **Persistenz** — Chunks und Vektoren in SQLite (`tiser.db`), Vektoren kompakt als
  float32-BLOB. Originaldokumente werden **nicht** gespeichert. BM25-Index wird beim
  Start neu aufgebaut.
- **Dubletten-Schutz** — Inhalts-Hash (SHA-1 des extrahierten Texts) pro Dokument:
  identischer Inhalt wird übersprungen, gleicher Name mit neuem Inhalt ersetzt.
...
Datenbankdatei `tiser.db` im Anwendungsordner.
...
Einlesen über die Oberfläche: Button **„Ordner data\ einlesen"**. Einzelne Dateien
können alternativ direkt über **„Laden"** hochgeladen werden.

| Fall                                   | „Ordner data\ einlesen" | „Laden"             |
|----------------------------------------|--------------------------|---------------------|
| Dateiname schon indiziert              | übersprungen             | Inhalt wird geprüft |
| gleicher Name, **geänderter** Inhalt   | übersprungen (!)         | alte Chunks ersetzt |
| gleicher Inhalt, anderer Name          | übersprungen (Dublette)  | übersprungen        |

> Der Ordner-Import prüft zuerst den Dateinamen (schnell, ohne Extraktion). Eine
> **geänderte** Datei mit gleichem Namen deshalb über „Laden" einlesen.
...
**Dokumente aktualisieren:** Neue Dateien in `data\` legen und erneut einlesen,
geänderte über „Laden". Für einen kompletten Neuaufbau `tiser.db` löschen und neu
einlesen — die
...
| `CTX_PREFIX`    | Dokumenttitel als Chunk-Vorspann (`False`)        |
| `CTX_TITLE_MAX` | max. Titellänge im Vorspann (60)                  |

> **`CTX_PREFIX`** stellt „Dokumenttitel > Überschrift:" vor jeden Chunk (Embedding
> und BM25). Standardmässig aus, bis eine Golden-Set-Messung den Nutzen belegt. Wirkt
> nur auf **neu** eingelesene Dokumente — zum Vergleichen jeweils `tiser.db` löschen
> und neu einlesen, sonst misst die Eval einen Mischkorpus.
...
- **Log meldet „Chunks mit abweichender Vektor-Dimension NICHT geladen"** → in der
  DB liegen Vektoren aus zwei verschiedenen Embedding-Modellen. `tiser.db` löschen
  und neu einlesen.
- **Fehler beim Start der Konsole** → Details in `tiser_error.log` im Anwendungsordner.
